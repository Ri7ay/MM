...
#define  MEMORY_SIZE_LITTLE 1000
#define  MEMORY_SIZE_BIG 30000


// 330к - 350к живет 500к итераций

int main (void) {
    int *mem_block[600000], block_lifetime[600000], no_memory = 0;

    for (int i = 0; i < 600000; i++) {
        // Время жизни блоков = 0
        block_lifetime[i] = 0;
    }
    ...
    for(int i = 0; i < 600000; i++) {
        ...
        for (int j = 0; j < i + 1; j++) {
            ...
            if (block_lifetime[j] == 0) {
                // Если время жизни = 0, освобождаю память
                my_free(mem_block[j]);
                mem_block[j] = NULL;
            }
        }

        mem_block[i] = my_malloc(rand() % 1000 + 1); // Выделяю память
        // Если памяти хватило даю ей вермя жизни, если нет увеличиваю счетчик
        mem_block[i] != NULL ? (block_lifetime[i] = rand() % 1000 + 1) : (no_memory++);
        /*
        printf("\n");
...
int main (void) {
    ...
    *((int**)(end_mem + 3)) = free_block[9];
    ...
    for(int i = 0; i < 1000; i++) {
        ...
        mem_block[i] = my_malloc(rand() % 1000 + 16); // Выделяю память
        // Если памяти хватило даю ей вермя жизни, если нет увеличиваю счетчик
        mem_block[i] != NULL ? (block_lifetime[i] = rand() % 500 + 1) : (no_memory++);
        printf("\n");
